...
export async function scrapeUrls(urls, onProgress = null) {
    ...
    try {
        ...
        const crawler = createCrawler({
            ...
            requestHandler: async ({ page, request }) => {
                try {
                    ...
                    failed.push({ url: request.url, error: error.message });
                }
            ...
            failedRequestHandler: async ({ request }) => {
                ...
                failed.push({ url: request.url, error: 'Request failed' });
                processed++;
                if (onProgress) {
                    onProgress(processed, total);
                }
        ...
        // Cleanup RequestQueue storage
        await requestQueue.drop();
    }

    if (failed.length > 0) {
        console.warn(`Scraping completed with ${failed.length} failures:`, failed.map(f => f.url));
    }
